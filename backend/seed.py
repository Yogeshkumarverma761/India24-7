...
def seed_db():
    ...
    with Session(engine) as session:
        ...
        if session.exec(select(Citizen)).first():
            print("DB already seeded. Skipping.")
            return

        # Citizens
        citizens_list = [
            Citizen(username="rahul88", email="rahul@example.com", hashed_password="hashed_pass_here", name="Rahul S.", avatar_url="RS", points=1540, badge="🥇"),
            Citizen(username="sneha_v", email="sneha@example.com", hashed_password="hashed_pass_here", name="Sneha V.", points=1980, badge="🥇"),
            Citizen(username="officer1", email="officer1@gov.in", hashed_password="hashed_pass_here", name="Officer Sharma", is_officer=True)
        ]
        
        for u in citizens_list:
            session.add(u)
        ...
        c1 = Complaint(
            ...
            upvotes=42, comments_count=5, creator_id=citizens_list[0].id, escalation_level=EscalationEnum.L1
        )
        ...
        c2 = Complaint(
            ...
            upvotes=120, comments_count=14, creator_id=citizens_list[1].id, escalation_level=EscalationEnum.L2, 
